...
static inline void faac_s16_deinterleave2_to_double_neon(double *dst, const int16_t *src_interleaved, unsigned int n, int which)
{
    ...
    unsigned int i = 0;
    if (which != 0 && which != 1) {
        // Should never happen for 2ch, but keep it safe.
        for (i = 0; i < n; i++) {
    ...
    for (; i + 8 <= n; i += 8) {
        int16x8x2_t v = vld2q_s16(src_interleaved + i * 2);
        int16x8_t v16 = v.val[which];
...
#endif
						{
							short *input_channel = (short*)inputBuffer + map;
