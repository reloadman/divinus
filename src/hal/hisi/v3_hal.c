...
int v3_region_create(char handle, hal_rect rect, short opacity)
{
    ...
    v3_sys_bind dest = { .module = V3_SYS_MOD_VENC, .device = _v3_venc_dev, .channel = 0 };
    ...
    for (char i = 0; i < V3_VENC_CHN_NUM; i++) {
        if (!v3_state[i].enable) continue;
        dest.channel = i;
        if (!hal_osd_is_allowed_for_channel(&v3_state[i])) {
            v3_rgn.fnDetachChannel(handle, &dest);
            continue;
        }
...
int v3_video_create(char index, hal_vidconfig *config)
{
    ...
    channel.gop.mode = V3_VENC_GOPMODE_NORMALP;
    ...
    if (config->codec == HAL_VIDCODEC_JPG) {
        ...
        if (h264_plus) {
            channel.gop.mode = V3_VENC_GOPMODE_SMARTP;
